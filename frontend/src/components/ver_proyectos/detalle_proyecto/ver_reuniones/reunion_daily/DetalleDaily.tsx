...
import { 
    ...
    LogOut, ChevronDown, Calendar, User, Clock, MessageSquare, HelpCircle
} from 'lucide-react';
...
const DetalleDaily: React.FC = () => {
    ...
    const [sesiones, setSesiones] = useState<DailySession[]>([]);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                // Usamos la ruta /reuniones/ que es la que te funcionó anteriormente
                const res = await axios.get(`${API_URL}/reuniones/proyecto/${id}?tipo=11`);
            ...
            <main className="content">
                <nav className="nav-top">
                    <div className="title-section" style={{display: 'flex', alignItems: 'center'}}>
                        ...
                        <h1>Reuniones / <span style={{color: '#f57c00'}}>Daily Scrum</span></h1>
                    </div>
                ...
                <div className="planning-view-container" style={{padding: '20px'}}>
                    <div className="filter-bar" style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px'}}>
                        ...
                        {/* CONTADOR DE DAILIES CON ESTILO NARANJA */}
                        <div className="total-badge" style={{
                            ...
                            Dailies registrados: {sesionesFiltradas.length}
                        </div>
                    </div>
                        ...
                        <table className="vp-table">
                            <thead>
                                <tr>
                                    <th>Fecha</th>
                                    <th>Sprint</th>
                                    <th>Resumen / Avances de la Sesión</th>
                                </tr>
                            </thead>
                            <tbody>
                                {loading ? (
                                    ...
                                    sesionesFiltradas.map(sesion => (
                                        <tr key={sesion.reuId}>
                                            ...
                                            <td style={{width: '150px'}}>
                                                <span className="badge-sprint" style={{backgroundColor: '#eee', padding: '4px 10px', borderRadius: '4px', fontSize: '0.85rem', fontWeight: '500'}}>
                                            ...
                                            <td style={{textAlign: 'left', lineHeight: '1.6'}}>
                                                <div style={{display: 'flex', alignItems: 'flex-start', gap: '8px', color: '#444', fontSize: '0.95rem', whiteSpace: 'pre-wrap'}}>
                                                    <MessageSquare size={16} style={{marginTop: '4px', flexShrink: 0, color: '#f57c00'}}/>
                                                    <span>
                                                        {sesion.reuResumen || sesion.reuDescripcion || (
                                                            <em style={{color: '#999'}}>Sin resumen registrado</em>
                                                        )}
                                                    </span>
                                                </div>
                                            </td>
                                    ...
                                    <tr>
                                        <td colSpan={3} className="empty-msg" style={{padding: '40px', color: '#999', textAlign: 'center'}}>
                                            No se encontraron Dailies para los criterios seleccionados.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
