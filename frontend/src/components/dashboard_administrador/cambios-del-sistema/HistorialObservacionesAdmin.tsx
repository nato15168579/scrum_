...
type HistorialFilter = "visto" | "pendiente";
...
const HistorialObservacionesAdmin = () => {
  ...
  const [filter, setFilter] = useState<HistorialFilter>("visto");
  ...
  const fetchCambios = async (nextFilter: HistorialFilter) => {
    setLoading(true);

    try {
      const response = await fetch(
        `${API_URL}/cambios-sistema?estado=${nextFilter}&limit=300`,
      );
  ...
  const markAsObserved = async (id: number) => {
    ...
    try {
      ...
      setTimedAlert({
        ...
        description: "Se actualizo el historial correctamente.",
      });
    } catch (error) {
  ...
  if (loading) {
    return <div className="loading-screen">Cargando historial...</div>;
  }
          ...
          <section className="cambios-system-card">
            <header className="cambios-system-header">
              <div className="cambios-system-title">
                <h2>{filter === "visto" ? "Observaciones vistas" : "Pendiente"}</h2>
                <span>
                  Usa los botones para filtrar. Las observaciones pendientes se
                  pueden marcar como vistas.
                </span>
              </div>
              ...
              <div className="cambios-system-actions">
                <button
                  ...
                  onClick={() => navigate("/cambios-del-sistema")}
                >
                  <ChevronLeft size={16} />
                  Volver
                </button>
                ...
                <button
                  type="button"
                  className={`cambios-system-btn ${filter === "visto" ? "active" : ""}`}
                ...
                <button
                  type="button"
                  className={`cambios-system-btn ${filter === "pendiente" ? "active" : ""}`}
              ...
              <tbody>
                {cambios.length > 0 ? (
                  cambios.map((item) => {
                    ...
                    const canMark = filter === "pendiente";
                    return (
                      <tr key={item.id}>
                        ...
                        <td className="cambios-system-action-cell">
                          {canMark ? (
                            ...
                            <span className="cambios-system-cell-muted">Visto</span>
                          )}
                        </td>
                      </tr>
                    );
...
export default HistorialObservacionesAdmin;
