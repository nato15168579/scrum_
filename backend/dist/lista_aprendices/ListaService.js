...
let ListaService = class ListaService {
    ...
    normalizeFichasCargo(rawValue) {
        const raw = String(rawValue !== null && rawValue !== void 0 ? rawValue : '').trim();
        if (!raw)
            return [];
    ...
    async findAllInstructores() {
        await this.ensureFechaRegistroColumn();
        const hasEspecializacionColumn = await this.columnExists('usuario', 'usu_especializacion');
